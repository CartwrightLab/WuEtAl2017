...
if [ $endpos -gt $startpos ];then
  ( samtools mpileup -t SP -u -r 21:${startpos}-${endpos} -f /storage/b37_reference/human_g1k_v37.fasta ${infile[0]} ${infile[1]} ${infile[2]} | bcftools call ${mode} -C trio -S trio_config_CEU.txt -> ${outfile} )
fi
